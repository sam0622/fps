...
@export var start_thickness: float = 0.1:
	set(new_start_thickness):
		start_thickness = new_start_thickness
@export var end_thickness: float = 0.1:
	set(new_end_thickness):
		end_thickness = new_end_thickness
@export var corner_resolution: int = 5:
	set(new_corner_resolution):
		corner_resolution = new_corner_resolution
@export var cap_resolution: int = 5:
	set(new_cap_resolution):
		cap_resolution = new_cap_resolution
@export var draw_caps: bool = true:
	set(new_draw_caps):
		draw_caps = new_draw_caps
@export var draw_crners: bool = true:
	set(new_draw_crners):
		draw_crners = new_draw_crners
@export var use_global_coords: bool = true:
	set(new_use_global_coords):
		use_global_coords = new_use_global_coords
@export var tile_texture: bool = true:
	set(new_tile_texture):
		tile_texture = new_tile_texture
...
func _process(_delta):
	...
	for i in range(points.size() - 1):
		...
		var AB: Vector3 = B - A
		var orthogonalABStart: Vector3 = (
			(cameraOrigin - ((A + B) / 2)).cross(AB).normalized() * thickness
		)
		var orthogonalABEnd: Vector3 = (
			(cameraOrigin - ((A + B) / 2)).cross(AB).normalized() * nextThickness
		)
		...
		if i == points.size() - 2:
			...
			if draw_crners:
				...
				var BC = C - B
				var orthogonalBCStart = (
					(cameraOrigin - ((B + C) / 2)).cross(BC).normalized() * nextThickness
				)
...
func cap(center: Vector3, pivot: Vector3, thickness: float, cap_resolution: int):
	...
	for i in range(1, cap_resolution):
		vertex_array[i] = (
			center + (orthogonal.rotated(axis, lerp(0.0, PI, float(i) / cap_resolution)))
		)
